...
bd_mercados <- bd_mercados[order(bd_mercados$`n()`, decreasing = TRUE),]
...
Das **`r ativas`** empresas ativas, apenas **`r mercados_ativos`** possuem informação sobre o mercado no qual participam, enquanto **`r mercados_na`** empresas não informam sobre o tipo de mercado.
...
col_mercados <- bd_ativas[,12]

col_mercados$TP_MERC[is.na(col_mercados$TP_MERC)] <- "NÃO INFORMA"

bd_mercados$por <- paste0(round(as.numeric((bd_mercados$`n()`/sum(bd_mercados$`n()`))*100)),"%")

x <- barplot((sort(table(col_mercados),decreasing=T)), xlab = "Tipo de Mercado", ylab = "QTD", col="lightblue", ylim=c(0,max(bd_mercados$`n()`+50)), cex.names = 0.6)

text(x,labels=bd_mercados$por, col="black", cex=1.2, bd_mercados$`n()`+14)
...
bd_ca <- bd_ca[order(bd_ca$`n()`, decreasing = TRUE),]
...
col_ca$CONTROLE_ACIONARIO[is.na(col_ca$CONTROLE_ACIONARIO)] <- "NÃO INFORMA"

bd_ca$por <- paste0(round(as.numeric((bd_ca$`n()`/sum(bd_ca$`n()`))*100)),"%")

#bd_mercados$por <- round(as.numeric((bd_mercados$`n()`/sum(bd_mercados$`n()`))*100))

y <- barplot((sort(table(col_ca),decreasing=T)), xlab = "Controle Acionário", ylab = "QTD", col="lightblue", ylim=c(0,max(bd_ca$`n()`+150)), cex.names = 0.4)

text(y,labels=bd_ca$por, col="black", cex=1.2, bd_ca$`n()`+25)
```
...
atividades <- atividades[order(atividades$`n()`, decreasing = TRUE),]

atividades$pos <- c(1:nrow(atividades)) 
# muda o nome das colunas
names(atividades) <- c('Atividade','QTD', 'POSICAO')

#atividades <- atividades[order(atividades$QTD, decreasing = TRUE),]
```
...
atividades$por <- paste0(round(as.numeric((atividades$QTD/sum(atividades$QTD))*100)),"%")

ggplot(atividades[order(atividades$QTD, decreasing = TRUE),], aes(y = QTD, x = Atividade, fill = QTD)) +
  geom_bar(stat = "identity") + geom_label(aes(x = Atividade, y = QTD/2, label = por)) + geom_label(aes(x = Atividade, y = -3, label = POSICAO)) +
  coord_flip() 
```
...
Aqui **`r bd_estados_soma`** empresas estão distribuídas em **`r nrow(bd_estados)`** estados diferentes.
...
#bd_auditoras_ordenado$pos <- c(1:nrow(bd_auditoras_ordenado))

auditoras <- bd_auditoras_ordenado[1:5,]

# muda o nome das colunas

auditoras$por <- paste0(round(as.numeric((auditoras$`n()`/sum(auditoras$`n()`))*100)),"%")

names(auditoras) <- c('Auditora','QTD','Frequência')
```
...
#ggplot(auditoras, aes(y = QTD, x = Auditora, fill = QTD)) +
 # geom_bar(stat = "identity") + geom_label(aes(x = Auditora, y = QTD/2, label = por)) + geom_label(aes(x = Auditora, y = -3, label = POSICAO)) + coord_flip()
```
...
ativas_por <- round(as.numeric(ativas/todas_empresas*100))

# pricipal mercado
mercado_principal <- bd_mercados_total[1,1]
mercado_principal_qtd <- bd_mercados_total[1,2]
mercado_principal_por <- round(as.numeric(mercado_principal_qtd/ativas*100))

# principal controle acionário
ca_principal <- bd_ca_ativas[1,1]
ca_principal_qtd <- bd_ca_ativas[1,2]
ca_principal_por <- round(as.numeric(ca_principal_qtd/ativas*100))

# principal atividade
atividade_principal <- atividades[1,1]
atividade_principal_qtd <- atividades[1,2]
atividade_principal_por <- round(as.numeric(atividade_principal_qtd/ativas*100))

# principal estado
estado_principal <- bd_estados[1,1]
estado_principal_qtd <- bd_estados[1,2]
estado_principal_por <- round(as.numeric(estado_principal_qtd/ativas*100))

# principal auditora
auditora_principal <- bd_auditoras_ordenado[1,1]
auditora_principal_qtd <- bd_auditoras_ordenado[1,2]
auditora_principal_por <- round(as.numeric(auditora_principal_qtd/ativas*100))
```

Vimos que das **`r todas_empresas`** listadas na B3, **`r nrow(bd_ativas)` (`r ativas_por`%)** estão com o registro Ativo; destas, **`r mercado_principal_qtd` (`r mercado_principal_por`%)** estão presentes no(a) `r mercado_principal`, **`r ca_principal_qtd` (`r ca_principal_por`%)** são de controle `r ca_principal`, **`r atividade_principal_qtd` (`r atividade_principal_por`%)** são do setor de `r atividade_principal`, **`r estado_principal_qtd` (`r estado_principal_por`%)** concentram-se no estado de(o/a) `r estado_principal` e **`r auditora_principal_qtd` (`r auditora_principal_por`%)** são auditadas pela `r auditora_principal`.



## Caso ainda queira verificar informações específicas das empresas, é só consultar a tabela abaixo:

```{r}
#| echo: false
bd |> reactable::reactable(filterable = TRUE)
```
...
-   [Informações no site da B3](https://www.b3.com.br/pt_br/)

-   [Créditos da Imagem](https://financasinteligentes.com/blog/b3-o-que-e-e-como-funciona-a-bolsa-de-valores/)
